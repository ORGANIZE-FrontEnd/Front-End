...
export const calculateSummary = (
  ...
  if (filterType === "day") {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      incomesByPeriod[dayLabel] = transactions.incomes.reduce((sum: number, item: { date: string; price: number }) => {
        const itemDate = normalizeDate(item.date);
        return itemDate.getDate() === day && itemDate.getMonth() === selectedDate.getMonth() && itemDate.getFullYear() === selectedDate.getFullYear()
          ? sum + item.price
          : sum;
      }, 0);

      expensesByPeriod[dayLabel] = transactions.expenses.reduce((sum: number, item: { date: string; price: number }) => {
        const itemDate = normalizeDate(item.date);
        return itemDate.getDate() === day && itemDate.getMonth() === selectedDate.getMonth() && itemDate.getFullYear() === selectedDate.getFullYear()
          ? sum + item.price
          : sum;
      ...
      balanceByPeriod[dayLabel] = calculateCumulativeBalance(transactions, dayDate);
    }
    ...
    while (currentWeekStart.getMonth() === month || currentWeekStart <= firstDayOfMonth) {
      ...
      incomesByPeriod[weekLabel] = transactions.incomes.reduce((sum: number, item: { date: string; price: number }) => {
        const itemDate = normalizeDate(item.date);
        return itemDate >= currentWeekStart && itemDate <= weekEnd ? sum + item.price : sum;
      }, 0);

      expensesByPeriod[weekLabel] = transactions.expenses.reduce((sum: number, item: { date: string; price: number }) => {
        const itemDate = normalizeDate(item.date);
        return itemDate >= currentWeekStart && itemDate <= weekEnd ? sum + item.price : sum;
      }, 0);

      resultsByPeriod[weekLabel] = incomesByPeriod[weekLabel] - expensesByPeriod[weekLabel];
      balanceByPeriod[weekLabel] = calculateCumulativeBalance(transactions, weekEnd);

      currentWeekStart.setDate(currentWeekStart.getDate() + 7);
    }
  } else if (filterType === "month") {
    const monthLabel = formatDate(selectedDate.toISOString(), "monthYear");
    incomesByPeriod[monthLabel] = transactions.incomes.reduce((sum: number, item: { date: string; price: number }) => {
      const itemDate = normalizeDate(item.date);
      return itemDate.getMonth() === selectedDate.getMonth() && itemDate.getFullYear() === selectedDate.getFullYear() ? sum + item.price : sum;
    }, 0);

    expensesByPeriod[monthLabel] = transactions.expenses.reduce((sum: number, item: { date: string; price: number }) => {
      const itemDate = normalizeDate(item.date);
      return itemDate.getMonth() === selectedDate.getMonth() && itemDate.getFullYear() === selectedDate.getFullYear() ? sum + item.price : sum;
    }, 0);

    resultsByPeriod[monthLabel] = incomesByPeriod[monthLabel] - expensesByPeriod[monthLabel];
    balanceByPeriod[monthLabel] = calculateCumulativeBalance(transactions, new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0));
  }
